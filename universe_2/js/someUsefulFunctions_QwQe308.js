...
function antiExpRootSoftcap(num,start,power){
    if(num.lte(start)) return num;
    return ten.pow(num.log10().div(start.log10().pow(one.sub(one.div(power)))).pow(power))
}
//修改class属性
...
function hasUpgThenDiv(UPGlayer,id,num){
    if(hasUpgrade(UPGlayer,id)) return num.div(tmp[UPGlayer].upgrades[id].effect)
    return num
}
...
function hasMSThenAdd(UPGlayer,id,num){
    if(hasMilestone(UPGlayer,id)) return num.add(tmp[UPGlayer].milestones[id].effect)
    return num
}
function hasMSThenMul(UPGlayer,id,num){
    if(hasMilestone(UPGlayer,id)) return num.mul(tmp[UPGlayer].milestones[id].effect)
    return num
}
function hasMSThenPow(UPGlayer,id,num){
    if(hasMilestone(UPGlayer,id)) return num.pow(tmp[UPGlayer].milestones[id].effect)
    return num
}