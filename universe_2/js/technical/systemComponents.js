var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="player.points.lt('1e1000')"  class="overlayThing">{{options.ch?'您前行了':'You Have Walked'}} </span>
		<h2  class="overlayThing" id="points">{{format(player.points)}}</h2>
		<span v-if="player.points.lt('1e1e6')"  class="overlayThing"> {{options.ch?modInfo.pointsName:modInfo.pointsNameEN}}</span>
		<br>
		<span v-if="tmp.other.oompsMag != 0"  class="overlayThing">({{format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s"}}/sec)</span>
